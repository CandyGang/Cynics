...
from Cynics.helpers.pyrohelper import get_args
...
__help__ = f"""
`{PREFIX}kang`: help in kanging sticker
`{PREFIX}strinfo`: fetch info of stickers
"""

@CynicsCli.on_message(filters.command("kang", PREFIX) & filters.me)
async def kang(client, message):
    user = await CynicsCli.get_me()
    ...
    if replied and replied.media:
        ...
        photo = await CynicsCli.download_media(message=replied)
    else:
    ...
    if photo:
        ...
        packname = f"a{user.id}_by_Cynics_{pack}"
        ...
        if exist is not False:
            try:
                await CynicsCli.send_message("Stickers", "/addsticker")
            except YouBlockedUser:
                await message.edit("first **unblock** @Stickers")
                return
            await CynicsCli.send_message("Stickers", packname)
            ...
            while limit in await get_response(message):
                ...
                if await get_response(message) == "Invalid pack selected":
                    await CynicsCli.send_message("Stickers", cmd)
                    await get_response(message)
                    await CynicsCli.send_message("Stickers", packnick)
                    await get_response(message)
                    await CynicsCli.send_document("Stickers", photo)
                    await get_response(message)
                    await CynicsCli.send_message("Stickers", emoji_)
                    await get_response(message)
                    await CynicsCli.send_message("Stickers", "/publish")
                    if is_anim:
                        await get_response(message)
                        await CynicsCli.send_message(
                            "Stickers", f"<{packnick}>", parse_mode=None
                        )
                    await get_response(message)
                    await CynicsCli.send_message("Stickers", "/skip")
                    await get_response(message)
                    await CynicsCli.send_message("Stickers", packname)
            ...
            await CynicsCli.send_message("Stickers", emoji_)
            await get_response(message)
            await CynicsCli.send_message("Stickers", "/done")
        else:
            ...
            if is_anim:
                await get_response(message)
                await CynicsCli.send_message("Stickers", f"<{packnick}>", parse_mode=None)
            await get_response(message)
            await CynicsCli.send_message("Stickers", "/skip")
            await get_response(message)
            await CynicsCli.send_message("Stickers", packname)
        ...
        await CynicsCli.read_history("Stickers")
        if os.path.exists(str(photo)):
            os.remove(photo)


@CynicsCli.on_message(filters.command("stkrinfo", PREFIX) & filters.me)
async def sticker_pack_info_(client, message):
    ...
    get_stickerset = await CynicsCli.send(
        GetStickerSet(
    ...
    for document_sticker in get_stickerset.packs:
        if document_sticker.emoticon not in pack_emojis:
            pack_emojis.CynicsCliend(document_sticker.emoticon)
    out_str = (
...
async def get_response(message):
    return [x async for x in CynicsCli.iter_history("Stickers", limit=1)][0].text
